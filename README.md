...
9. The project and data structure should have as few nesting levels as possible, since hierarchy reduces the visibility of elements (a file located too deep can be forgotten), nesting also increases the length of the access chain to elements (more checks for the presence of an element will be required), at the same time, it is worth considering that too flat a structure can make the project chaotic.
